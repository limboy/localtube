  ...
  (
    ...
    return (
      ...
      >
        ...
        <div
          className={cn(
            "fixed inset-y-0 z-20 h-svh w-(--sidebar-width) transition-[left,right,width] duration-150 ease-in-out flex",
            side === "left"
...
>(({ className, onClick, ...props }, ref) => {
  ...
  return (
    <Button
      ...
      variant="ghost"
      size="icon"
      className={cn("h-7 w-7 [app-region:no-drag] [-webkit-app-region:no-drag]", className)}
      onClick={(event) => {
        onClick?.(event);
        toggleSidebar();
      }}
  ...
  ({ className, ...props }, ref) => {
    return (
      <div
        ref={ref}
        data-sidebar="group"
        className={cn("relative flex w-full min-w-0 flex-col p-2 pb-0", className)}
        {...props}
      />
    );
  }
...
const SidebarGroupContent = React.forwardRef<HTMLDivElement, React.ComponentProps<"div">>(
  ({ className, ...props }, ref) => (
    <div
      ...
      className={cn("w-full text-sm overflow-x-hidden p-0", className)}
      {...props}
    />
  )
);
...
const sidebarMenuButtonVariants = cva(
  "peer/menu-button flex w-full items-center gap-2 overflow-hidden rounded-md p-2 text-left text-sm outline-none ring-0 transition-[width,height,padding] hover:bg-sidebar-accent hover:text-sidebar-accent-foreground active:bg-sidebar-accent active:text-sidebar-accent-foreground disabled:pointer-events-none disabled:opacity-50 group-has-[[data-sidebar=menu-action]]/menu-item:pr-8 group-has-[[data-sidebar=menu-badge]]/menu-item:pr-10 aria-disabled:pointer-events-none aria-disabled:opacity-50 data-[active=true]:bg-sidebar-accent data-[active=true]:font-medium data-[active=true]:text-sidebar-accent-foreground data-[state=open]:hover:bg-sidebar-accent data-[state=open]:hover:text-sidebar-accent-foreground group-data-[collapsible=icon]:!size-8 group-data-[collapsible=icon]:!p-2 [&>span:last-child]:truncate [&>svg]:size-4 [&>svg]:shrink-0",
  {
    variants: {
      variant: {
